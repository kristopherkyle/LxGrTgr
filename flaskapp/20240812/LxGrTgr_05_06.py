...
def complexity(token,sent):
	...
	if token.cxtag == "vb+that+compcls" and sent[token.headidx].lemma == "be" and "acomp" in [x.deprel for x in sent if x.headidx == token.headidx]:		
		token.cxtag = "xtrapos+jj+that+jcomp"
...
# printer(tag("It is evident that the virus formation is related to the cytoplasmic inclusions.")) #xtrapos+jj+that+compcls #spacy mistags this as a verb complement
