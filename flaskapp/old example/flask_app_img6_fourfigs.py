...
def rundata(text,lang):
	#print(text)
	if lang == "Eng":
		prms = lats.ld_params_en_trf
	if lang == "Span":
		prms = lats.parameters_es_trf #note that these are not lemmatized. Need to make new parameter set for this.
	#this will be added in a future release of pylats
	# if lang == "Other":
	# 	prms = lats.parameters_whtsp
	outd = {}
	if len(text) == 0:
		...
		tokenized = lats.Normalize(text, params = prms)
		...
		#MATTR 50
		outd["name1"] = "MATTR 50"
		outd["value1"] = round(output.mattr,4) #round output to 4 decimal places
		outd["image1"] = kde(output.mattrs)

		#MATTR 11
		outd["name2"] = "MATTR 11"
		outd["value2"] = round(output.mattr11,4) #round output to 4 decimal places
		outd["image2"] = kde(output.mattr11s)

		#MTLD .72
		outd["name3"] = "MTLD .72"
		outd["value3"] = round(output.mtld,4) #round output to 4 decimal places
		outd["image3"] = kde(output.mtldvals)

		#MTLD .92
		outd["name4"] = "MTLD .92"
		outd["value4"] = round(output.mtld92,4) #round output to 4 decimal places
		outd["image4"] = kde(output.mtld92vals)
...
def index():
	...
	if request.method == "POST": #if we have data to work with
		processed = rundata(request.form["contents"],request.form["language"]) #process data from form using the rundata() function
		return render_template("main_index11_fourfigs.html", items = [processed])
	else: #this is the landing page.

		return render_template("landing_opts.html")
