...
class MyApp: #this is the class for the gui and the text analysis
	def __init__(self, parent):
		...
		self.spacer1= tk.Label(parent, text= "Lexicogrammatical Tagger", font = helv16, background = color)
		self.spacer1.pack()
		...
		# self.options_frame = tk.LabelFrame(self.myContainer1, text= "Target Language", background = color)
		# self.options_frame.pack(fill = tk.X,expand=tk.TRUE)
		
		#insert radiobutton here
		# self.lang_var = tk.StringVar()
		# self.lang_en = tk.Radiobutton(self.options_frame, text="English", value = "english", variable=self.lang_var,background = color)
		# self.lang_en.grid(row=1,column=1, sticky = "W")
		# self.lang_es = tk.Radiobutton(self.options_frame, text="Spanish", value = "spanish", variable=self.lang_var,background = color)
		# self.lang_es.grid(row=1,column=2, sticky = "W")
		# self.lang_ko = tk.Radiobutton(self.options_frame, text="Korean", value = "korean", variable=self.lang_var,background = color)
		# self.lang_ko.grid(row=1,column=3, sticky = "W")
		# #self.lang_en.grid(row=1,column=1, sticky = "W")	
		# self.var_dict["lang"] = self.lang_var
		
		self.secondframe= tk.LabelFrame(self.myContainer1, text= "Tag Folder of Texts", background = color)
		...
		self.outputdirlabel = tk.LabelFrame(self.secondframe, height = "1", width= "45", padx = "4", text = "Your selected output folder:", background = color)
	...
	def instruct_mess(self, event):
		messagebox.showinfo("Instructions", "1. Choose the input folder (where your files are)\n\n2. Choose the location where your tagged files should be written\n\n3. Press the 'Process Texts' button")
	...
	def button2Click(self, event):
		...
		self.outdirname = tkinter.filedialog.askdirectory(parent=root,initialdir="/",title='Please select a directory')
...
def main(indir, outdir, var_dict):
	...
	if problem == False:
		# dataQueue.put("Loading Transformer Model (en_core_web_trf)")
		# print("Loading Transformer Model (en_core_web_trf)")
		# lxgr.nlp = lxgr.spacy.load("en_core_web_trf")
		dataQueue.put("Loading Spacy")
		import lxgrtgr as lxgr #double check this works
		dataQueue.put("Processing your files")
		lxgr.tagFolder(indir,outdir)
		#lxcx.fullprocess(indir,var_dict["lang"].get(),outdir,var_dict["indout"].get())
