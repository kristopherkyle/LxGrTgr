...
prog_name = "LxGrTagger Tag Counter v0.3"
...
def main(indir, outdir, var_dict):
	...
	if problem == False:
		...
		#add tag lists
		CountDictionary = tc.countTagsFolder(indir)
		print(var_dict["norm"].get())
		if var_dict["norm"].get() == 0:
			tc.writeCounts(CountDictionary,outdir,normed = False)
		else:
			tc.writeCounts(CountDictionary,outdir,norming = var_dict["norm"].get())
