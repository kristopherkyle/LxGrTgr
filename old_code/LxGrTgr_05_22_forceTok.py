...
### imports ####################################
version = "0.0.5.22"
version_notes = "0.0.5.22 - Address appositive issues"
...
def complexity(token,sent):
	...
	if token.lxgrtag in ["nn"] and token.xpos not in ["NNP","NNPS"] and token.cat3 in ["nappos"]:
		if token.lemma not in citations and sent[token.headidx].lemma not in citations:
