...
### imports ####################################
version = "0.0.5.18"
version_notes = "0.0.5.18 - work on thatcls+vcomp (primarily questions and quoted speech)"
...
def complexity(token,sent):
	...
	#updated 2024-02-14
	if token.cxtag in ["thatcls+vcomp","whcls+vcomp"] and sent[token.headidx].lemma == "be" and "acomp" in [x.deprel for x in sent if x.headidx == token.headidx]:		
	...
	if token.lxgrtag in ["rb"] and token.lemma not in ["as"]: #"as" to avoid comparatives. May need to add others.
		...
		if sent[token.headidx].xpos in ["RB"] and token.deprel in ["advmod"]: 
			token.cxtag = "rb+jjrbmod"
