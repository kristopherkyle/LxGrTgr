...
### imports ####################################
version = "0.0.5.5"
version_notes = "0.0.5.5 - Continue integrating Biber et al.'s complexity Tags. Many bug fixes"
...
def complexity(token,sent):
	...
	if token.cat5 in ["compcls"] and token.cat6 in ["tocls"]: 
			if token.cat7 in ["jcomp", "vcomp"] and sent[token.headidx].lemma == "be" and "acomp" in [x.deprel for x in sent if x.headidx == token.headidx]: #the "jcomp" might not be necessary
				token.cxtag = "xtrapos+adj+to+jcomp"
			elif token.cat7 in ["jcomp"]:
				token.cxtag = "adj+to+jcomp"
...
##### Samples/tests
printer(tag("McKenna wrote about the origins of human language")) #start here
# printer(tag("The aviation security committee has convened."))
# printer(tag("He is away for fighter pilot training"))
# printer(tag("These are the conventional practices."))
# printer(tag("I suffered an emotional injury"))
# printer(tag("Alright, we'll talk to you in the morning."))
# printer(tag("I raved about it afterwards."))
# printer(tag("The formula for calculating the effective resistance is complicated."))
# printer(tag("It was important to obtain customer feedback."))
# printer(tag("I was happy to do it."))
# printer(tag("The project is part of a massive plan to complete the section of road."))
# printer(tag("You’re the best person to ask."))
# printer(tag("Elevated levels are treated with a diet consisting of low cholesterol foods."))
# printer(tag("This is a phrase that is used in the recruitment industry."))
# printer(tag("This is a phrase used in the recruitment industry."))
# printer(tag("I like watching the traffic go by."))
# printer(tag("I really want to fix this room up."))
# printer(tag("Based on estimates of the number of unidentified species, other studies put the sum total in the millions."))
# printer(tag("Considering mammals’ level of physical development, the diversity of this species is astounding."))
# printer(tag("Sections of fixed cells were examined to verify this hypothesis.")) # to+purpose+adverbial
# printer(tag("To verify this hypothesis, sections of fixed cells were examined.")) # to+purpose+adverbial
# printer(tag("I’ll offer a suggestion for what we should do."))
# printer(tag("It is evident that the virus formation is related to the cytoplasmic inclusions.")) #need to figure out extraposed clauses
# printer(tag("We’re happy that the hunger strike has ended.")) #adj+that+ccomp
# printer(tag("The fact that no tracer particles were found indicates that these areas are not a pathway.")) #noun+that+ncomp
# printer(tag("We need to account for the experimental error that could result from using cloze tests.")) #noun+that+relcl
# printer(tag("I don’t know how they do it.")) #verb+wh+ccomp
# printer(tag("yeah, I think I probably could")) #verb+that+ccomp. FIXED PROBLEM: But, "could" is not being counted as a main verb, which is throwing everything off.
# printer(tag("I would hope that we can have more control over them.")) #verb+that+ccomp #note, I fixed a vcomp issue
